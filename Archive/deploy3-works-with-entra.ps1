...
Write-Log "aadUserPrincipalName: $aadUserPrincipalName"
...
try {
  ...
  do {
    ...
    if ($provisioningState -eq "Succeeded") {
      ...
      # Extract server name from outputs
  ...
  if ($provisioningState -eq "Succeeded") {
    ...
    try {
      ...
      Write-Log "Warning: Failed to configure Microsoft Entra admin - $($_.Exception.Message)"
      if ($_.Exception.Response) {
        try {
          $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
          $responseBody = $reader.ReadToEnd()
      ...
      # Don't fail the entire script if admin configuration fails
    }
  }
